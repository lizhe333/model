...
HYDRA_MODEL_PATH = PACKAGE_ROOT / "configs/hydra/model/side_model3_adapter_v1.yaml"
...
def build_preflight(config: SideModel3AdapterConfig | None = None) -> dict[str, Any]:
    ...
    if hydra_model.get("_target_") != "side_model3_adapter.runtime.create_side_model3_adapter_wam":
...
def launch(
    ...
    *,
    ...
    if not dry_run:
        raise ContractError(
            "Side-Model3-Adapter has no registered suite or formal execution contract; "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Side-Model3-Adapter v1 wiring")
    parser.add_argument(
        "--config",
        type=Path,
        default=None,
    ...
    if not args.dry_run:
        parser.error("Side-Model3-Adapter currently exposes only --dry-run preflight")
