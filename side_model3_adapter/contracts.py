"""Fail-fast checks for the copied Side-Model3-Adapter v1 contract."""
...
from .config import METHOD_ID, SideModel3AdapterConfig, default_config


class ContractError(ValueError):
    """Raised when a configuration or live model is not Side-Model3-Adapter v1."""
...
def validate_training_data_config(data_cfg: Any) -> dict[str, Any]:
    ...
    _require(
        ...
        "Side-Model3-Adapter training requires raw video instead of latent cache",
        errors,
    )
    _require(
...
def _validate_live_model(model: Any, config: SideModel3AdapterConfig, errors: list[str]) -> None:
    architecture = config.architecture
    _require(type(model).__name__ == "SideModel3AdapterWAM", "wrong live model class", errors)
...
def validate_contract(
    config: SideModel3AdapterConfig | None = None,
    *,
    model: Any | None = None,
) -> dict[str, Any]:
    ...
    _require(
        config.track_id == "side_model3_adapter",
        "track_id must be side_model3_adapter",
        errors,
    )
    _require(
        config.method_id == METHOD_ID,
        "method_id must identify Side-Model3-Adapter v1",
        errors,
    )
    _require(
        config.runtime_package == "side_model3_adapter",
        "runtime package must be side_model3_adapter",
        errors,
    )
    _require(config.hydra_model == "side_model3_adapter_v1", "wrong Hydra model name", errors)
    ...
    _require(
        ...
        "Side-Model3-Adapter v1 requires Wan2.1-T2V-1.3B",
        errors,
    )
