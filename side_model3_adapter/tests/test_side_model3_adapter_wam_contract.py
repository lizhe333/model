"""Focused orchestration contracts for a lightweight SideModel3AdapterWAM shell."""
...
from side_model3_adapter.models.ema_target import (
    EMATargetPredictiveEncoder,
    EMATargetWanAdapters,
)
from side_model3_adapter.models.future_latent_change_head import (
    MultiHorizonFutureLatentChangeHead,
)
from side_model3_adapter.models.ladder_side_encoder import LadderSideEncoder
from side_model3_adapter.models.latent_transition import (
    LatentTransitionPredictor,
    MultiHorizonActionChunkEncoder,
)
from side_model3_adapter.models.side_model3_adapter_wam import SideModel3AdapterWAM
from side_model3_adapter.models.visual_anchor_resampler import (
    VisualAnchorActionFusion,
    VisualAnchorResampler,
)
from side_model3_adapter.runtime import create_side_model3_adapter_wam
...
def test_factory_rejects_non_v1_adapter_routes(mutation, message) -> None:
    ...
    with pytest.raises(ValueError, match=message):
        create_side_model3_adapter_wam(**kwargs)
...
def _make_lightweight_wam(
    *, loss_weights: dict[str, float] | None = None
    ...
    model = SideModel3AdapterWAM.__new__(SideModel3AdapterWAM)
    ...
    model.online_predictive_encoder = LadderSideEncoder(
        ...
        layer_indices=SideModel3AdapterWAM.selected_wan_layers,
        num_slots=NUM_SLOTS,
        ...
        for layer, state in zip(self.selected_wan_layers, states):
            if layer in self.adapter_layer_indices:
                if use_target_adapters:
                    state = self.target_wan_adapters.apply(layer, state)
                else:
...
def test_side_model3_adapter_checkpoint_round_trip_and_identity_guard(tmp_path) -> None:
    model = _make_lightweight_wam()
    checkpoint_path = tmp_path / "side_model3_adapter.pt"
    ...
    assert payload["method_id"] == SideModel3AdapterWAM.method_id
    assert payload["model_class"] == "SideModel3AdapterWAM"
