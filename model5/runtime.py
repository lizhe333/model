"""Hydra-facing factory for the top-level model5 package."""
...
def _as_dict(name: str, value: Any, *, required: bool = False) -> dict[str, Any]:
    ...
    if value is None:
        if required:
            raise ValueError(f"`{name}` is required for model5.")
        return {}
...
def create_model5_wam(
    *,
    ...
    if legacy_state_fusion:
        raise ValueError(
            "Model5 cannot consume a StateFusion action-expert config; "
