...
def _minimal_model5() -> Model5WAM:
    ...
    model.action_feature_temporal_scope = "current_plus_noisy_future"
...
def test_model5_checkpoint_round_trip_and_rejects_a1_identity(tmp_path: Path):
    torch.manual_seed(3)
    source = _minimal_model5()
    checkpoint = tmp_path / "model5.pt"
    source.save_checkpoint(checkpoint, step=12)

    restored = _minimal_model5()
    payload = restored.load_checkpoint(checkpoint)
    assert payload["method_id"] == Model5WAM.method_id
...
def test_production_model5_action_policy_is_smaller_than_statefusion_a1():
