"""Validated launcher for the Model3-derived Model5 backend plugin."""
...
def build_backend_environment(config: Model5Config, backend_output: Path, run_id: str) -> dict[str, str]:
    ...
    return {
        ...
        "MOT_CHECKPOINT_MIXED_ATTN": "True" if training.gradient_checkpointing else "False",
        "RUN_ID": run_id,
        "RUN_TAG": f"model5_{suite}",
        "SUITE_NAME": suite,
        "OUTPUT_DIR": str(backend_output),
        "WANDB_PROJECT": "i003-model5",
        "WANDB_NAME": f"model5_{suite}_{run_id}",
        "WANDB_MODE": training.wandb_mode,
        "DATASET_DIR": str(data.dataset_dir),
        "LATENT_CACHE_DIR": str(data.latent_cache_dir),
        "TEXT_EMBED_CACHE_DIR": str(data.text_embedding_cache_dir),
        "TEMPORAL_PREFIX_STEPS": str(data.policy_action_horizon),
        "MODEL5_POLICY_ACTION_HORIZON": str(data.policy_action_horizon),
        "ACTION_FEATURE_TEMPORAL_SCOPE": architecture.action_feature_temporal_scope,
        "FIXED_FEATURE_TIMESTEP": str(architecture.fixed_feature_timestep),
        "FUTURE_FEATURE_LATENT_SLOTS": str(architecture.future_feature_latent_slots),
        "ACTION_FEATURE_SPATIAL_DOWNSAMPLE_FACTOR": str(
            architecture.action_feature_spatial_downsample_factor
        ),
...
def build_backend_command(config: Model5Config) -> list[str]:
    return [
        "conda",
        "run",
        "--no-capture-output",
...
def _prepare_run(config: Model5Config, config_path: Path, run_id: str) -> tuple[Path, Path, dict[str, Any]]:
    ...
    manifest: dict[str, Any] = {
        ...
        "method_lineage": "model3_asymmetric_tri_timestep_vla_query_b1",
    ...
    (evidence_dir / "run_report.md").write_text(
        f"# Model5 {suite_label} Training\n\n"
        "Status: starting.\n\n"
        f"Backend output: `{backend_output}`\n\n"
        f"Progress log: `{backend_output / 'logs' / 'training.log'}`\n",
        encoding="utf-8",
    )
...
def launch(config_path: Path, run_id: str, *, dry_run: bool) -> int:
    ...
    _write_json(evidence_dir / "run_manifest.json", manifest)
    (evidence_dir / "run_report.md").write_text(
        f"# Model5 {_suite_label(config)} Training\n\n"
        "Status: active.\n\n"
        f"Backend output: `{backend_output}`\n\n"
        f"Progress log: `{backend_output / 'logs' / 'training.log'}`\n",
        encoding="utf-8",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate and launch Model5 LIBERO training.")
