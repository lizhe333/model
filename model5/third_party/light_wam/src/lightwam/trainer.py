...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        smoke_validation_cfg = cfg.get("smoke_validation", {})
        self.smoke_validation_enabled = bool(
            smoke_validation_cfg.get("enabled", False)
        )
        self.smoke_validation_seed = int(smoke_validation_cfg.get("seed", 123))
        self.smoke_validation_action_inference_steps = int(
            smoke_validation_cfg.get("action_inference_steps", 2)
        )
        if self.smoke_validation_action_inference_steps <= 0:
            raise ValueError("smoke_validation.action_inference_steps must be positive")
        ...
        logger.info(
            "Smoke validation: enabled=%s seed=%d action_inference_steps=%d",
            self.smoke_validation_enabled,
            self.smoke_validation_seed,
            self.smoke_validation_action_inference_steps,
        )
    ...
    def _finalize_benchmark(
        self,
        *,
        final_step_metrics: dict[str, float | bool] | None = None,
    ) -> dict[str, object]:
        ...
        if final_step_metrics is not None:
            payload["final_step_metrics"] = {
                str(key): value if isinstance(value, bool) else float(value)
                for key, value in final_step_metrics.items()
            }
    ...
    @torch.no_grad()
    def _run_action_inference_smoke(self, sample: dict) -> dict[str, float | bool]:
        if self.accelerator.num_processes != 1:
            raise RuntimeError("Model5 action inference smoke requires one process.")
        model = self.accelerator.unwrap_model(self.model)
        if not hasattr(model, "infer_action_from_latents"):
            raise RuntimeError("Smoke model lacks infer_action_from_latents().")
        model.eval()
        ...
        with self.accelerator.autocast():
            ...
            if observation_latents is None:
                observation_latents = inputs["input_latents"][:, :, 0:1]
            infer_kwargs = {
                "observation_latents": observation_latents,
                "context": inputs["context"],
                "context_mask": inputs["context_mask"],
                "action_horizon": int(model.action_policy.action_horizon),
                "proprio": inputs.get("proprio"),
                "num_inference_steps": self.smoke_validation_action_inference_steps,
                "seed": self.smoke_validation_seed,
                "rand_device": "cpu",
            }
            first = model.infer_action_from_latents(**infer_kwargs)["action"]
            second = model.infer_action_from_latents(**infer_kwargs)["action"]

        expected_shape = (
            int(observation_latents.shape[0]),
            int(model.action_policy.action_horizon),
            int(model.action_policy.action_dim),
        )
        max_abs_repeat_error = float((first - second).abs().max().item())
        diagnostics = model.get_last_action_feature_diagnostics()
        return {
            "inference/action_finite": bool(torch.isfinite(first).all().item()),
            "inference/action_shape_ok": tuple(first.shape) == expected_shape,
            "inference/reproducible": max_abs_repeat_error == 0.0,
            "inference/max_abs_repeat_error": max_abs_repeat_error,
            "inference/action_l2": float(first.float().square().mean().sqrt().item()),
            "inference/latent_slots": float(diagnostics["latent_slots"]),
            "inference/future_slots": float(diagnostics["future_slots"]),
            "inference/hidden_tokens": float(
        ...
        while self.global_step < self.max_steps:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    gradient_smoke_metrics: dict[str, float | bool] = {}
                    if self.smoke_validation_enabled:
                        unwrapped_for_smoke = self.accelerator.unwrap_model(self.model)
                        if not hasattr(unwrapped_for_smoke, "gradient_smoke_summary"):
                            raise RuntimeError("Smoke model lacks gradient_smoke_summary().")
                        gradient_smoke_metrics = (
                            unwrapped_for_smoke.gradient_smoke_summary()
                        )
                    self.optimizer.step()
                    ...
                    if (
                        ...
                        benchmark_metrics: dict[str, float | bool] = {
                            "loss": global_loss,
                            "grad_norm": global_grad_norm,
                            "learning_rate": current_lr,
                            "optimizer_step_was_skipped": optimizer_step_was_skipped,
                        }
                        benchmark_metrics.update(global_loss_metrics)
                        benchmark_metrics.update(gradient_smoke_metrics)
                        if self.smoke_validation_enabled:
                            benchmark_metrics.update(
                                self._run_action_inference_smoke(sample)
                            )
                        self._finalize_benchmark(
                            final_step_metrics=benchmark_metrics
                        )
                        return
