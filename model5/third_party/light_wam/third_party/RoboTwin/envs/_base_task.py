...
class Base_Task(gym.Env):
    ...
    def _compose_eval_video_frame(self, obs):
    ...
    def _get_eval_video_frame(self, live=False):
        if live:
            self.cameras.update_picture()
            rgb = self.cameras.get_rgb()
            return self._compose_eval_video_frame(rgb)

        if "observation" not in self.now_obs:
            self.cameras.update_picture()
            rgb = self.cameras.get_rgb()
            return self._compose_eval_video_frame(rgb)

        return self._compose_eval_video_frame(self.now_obs["observation"])
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self.eval_video_fps = int(kwags.get("eval_video_fps", 10))
        self.eval_video_record_mode = str(kwags.get("eval_video_record_mode", "action_step")).strip().lower()
        self.eval_video_frame_stride = max(1, int(kwags.get("eval_video_frame_stride", 1)))
        self.eval_video_frame_idx = 0
        self.eval_video_ffmpeg = None
        if self.eval_video_record_mode not in {"action_step", "scene_step"}:
            raise ValueError(
                f"Unsupported eval_video_record_mode={self.eval_video_record_mode}. "
                "Expected one of: ['action_step', 'scene_step']."
            )
    ...
    def _write_eval_video_frame(self, force=False, live=False):
        if self.eval_video_path is None or self.eval_video_ffmpeg is None:
            return
        if force or self.eval_video_frame_idx % self.eval_video_frame_stride == 0:
            eval_frame = self._get_eval_video_frame(live=live)
            self.eval_video_ffmpeg.stdin.write(eval_frame.tobytes())
        self.eval_video_frame_idx += 1
    ...
    def take_action(self, action, action_type:Literal['qpos', 'ee']='qpos'):  # action_type: qpos or ee
        ...
        if self.eval_video_path is not None:
            if self.eval_video_record_mode == "action_step":
                self._write_eval_video_frame(live=True)
            elif self.eval_video_record_mode == "scene_step" and self.eval_video_frame_idx == 0:
                self._write_eval_video_frame(force=True, live=True)
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            self.scene.step()
            self._update_render()
            if self.eval_video_path is not None and self.eval_video_record_mode == "scene_step":
                self._write_eval_video_frame(live=True)
                
            if self.check_success():
                self.eval_success = True
                self.get_obs() # update obs
                if self.eval_video_path is not None and self.eval_video_record_mode == "action_step":
                    self._write_eval_video_frame(force=True, live=True)
                return
