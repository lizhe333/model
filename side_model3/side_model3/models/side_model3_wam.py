"""Side-Model3 v1 policy with a completely frozen current-only Wan."""
...
class SideModel3WAM(LightWAM):
    """Frozen-Wan control policy with external predictive robot dynamics."""

    method_id = "side_model3_frozen_wan_ladder_flow_v1"
    ...
    def from_wan21_pretrained(
        ...
        *,
        ...
        if video_dit_config is None or "text_dim" not in video_dit_config:
            raise ValueError("Side-Model3 requires video_dit_config with text_dim")
        ...
        video_config["use_wam_adapter"] = False
        ...
        if bool(getattr(video_expert, "use_wam_adapter", False)):
            raise ValueError("Side-Model3 must not instantiate Wan adapters")
        if bool(getattr(video_expert, "has_backbone_lora", lambda: False)()):
            raise ValueError("Side-Model3 must not instantiate Wan LoRA")
        ...
        if tuple(side_config.get("layer_indices", cls.selected_wan_layers)) != cls.selected_wan_layers:
            raise ValueError("Side-Model3 v1 requires Wan layers 8,16,20,24,29")
        side_config.update(
            {
        ...
        model.model_paths = {
            ...
            "action_dit_backbone": "SIDE_MODEL3_NATIVE_MODEL3_ARCHITECTURE",
        }
        return model
    ...
    @torch.no_grad()
    def _extract_frozen_wan_states(
        self,
        *,
        ...
        if observation_latents.ndim != 5 or int(observation_latents.shape[2]) != 1:
            raise ValueError("Side-Model3 Wan input must be [B,C,1,H,W]")
        if bool(getattr(self.video_expert, "use_wam_adapter", False)):
            raise RuntimeError("Side-Model3 found a Wan adapter at runtime")
        if bool(getattr(self.video_expert, "has_backbone_lora", lambda: False)()):
            raise RuntimeError("Side-Model3 found Wan LoRA at runtime")

        captured: dict[int, torch.Tensor] = {}
        handles = []
        for layer_index in self.selected_wan_layers:
            def capture(_module, _inputs, output, *, index=layer_index):
                captured[index] = output.detach()

            handles.append(
                self.video_expert.blocks[layer_index].register_forward_hook(capture)
            )
        try:
            ...
            for handle in handles:
                handle.remove()
        return [captured[index].detach() for index in self.selected_wan_layers]
    ...
    def build_inputs(self, sample, tiled: bool = False) -> dict[str, Any]:
        ...
        if video is None:
            if sample.get("video_latents") is not None:
                raise ValueError(
                    "Side-Model3 requires raw video so t/t+4/t+8 are encoded independently"
                )
            raise ValueError("Side-Model3 training requires sample['video']")
        ...
        if context is None or context_mask is None or action is None or proprio is None:
            raise ValueError("Side-Model3 requires context, context_mask, action, and proprio")
        if action.ndim != 3 or int(action.shape[1]) < 8:
            raise ValueError("Side-Model3 requires at least eight expert actions")
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        current_wan_states = self._extract_frozen_wan_states(
        ...
        for horizon in self.target_action_offsets:
            ...
            with torch.no_grad():
                future_wan_states = self._extract_frozen_wan_states(
    ...
    def infer_action(
        ...
        self.eval()
        if proprio is None:
            raise ValueError("Side-Model3 requires current proprioception")
        if proprio.ndim == 1:
        ...
        wan_states = self._extract_frozen_wan_states(
            observation_latents=observation_latents,
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        if payload.get("method_id") != self.method_id:
            raise ValueError("Side-Model3 checkpoint method identity mismatch")
        if payload.get("model_class") != type(self).__name__:
            raise ValueError("Side-Model3 checkpoint class identity mismatch")
        expected_config = self.checkpoint_config()
        if payload.get("checkpoint_config") != expected_config:
            raise ValueError("Side-Model3 checkpoint tensor contract mismatch")
    ...
    def log_parameter_summary(self):
        ...
        for name in module_names:
            ...
            logger.info(
                "Side-Model3 module=%s total=%s trainable=%s",
                name,
