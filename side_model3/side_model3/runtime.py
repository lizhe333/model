"""Hydra-facing factory for the independent Side-Model3 v1 package."""
...
from .models import SideModel3WAM
...
def _as_dict(name: str, value: Any, *, required: bool = False) -> dict[str, Any]:
    ...
    if value is None:
        if required:
            raise ValueError(f"`{name}` is required for Side-Model3")
        return {}
...
def create_side_model3_wam(
    *,
    ...
    """Construct Side-Model3 while refusing every inherited Wan adaptation path."""
    ...
    if state_fusion:
        raise ValueError("Side-Model3 cannot instantiate StateFusion")
    if bool(adapter_config.get("use_wam_adapter", False)):
        raise ValueError("Side-Model3 cannot instantiate a Wan residual adapter")
    if bool(adapter_config.get("use_backbone_lora", False)):
        raise ValueError("Side-Model3 cannot instantiate Wan LoRA")
    if bool(video_dit.get("use_wam_adapter", False)):
        raise ValueError("video_dit_config cannot enable a Wan residual adapter")
    ...
    _require_value(
        ...
        "Side-Model3 v1 requires first_frame_causal Wan attention",
    )

    _require_value(model_id == "Wan-AI/Wan2.1-T2V-1.3B", "Side-Model3 v1 requires Wan2.1-T2V-1.3B")
    _require_value(video_backbone_type == "wan2_1_t2v", "Side-Model3 v1 requires the wan2_1_t2v backend")
    _require_value(
    ...
    _require_value(
        ...
        == (8, 16, 20, 24, 29),
        "Side-Model3 v1 requires Wan layers 8,16,20,24,29",
    )
    _require_value(int(side_encoder.get("num_slots", -1)) == 64, "Side-Model3 requires 64 control slots")
    ...
    _require_value(
        ...
        "Side-Model3 v1 keeps Action-DiT checkpointing disabled",
    )
    ...
    _require_value(
        ...
        "Side-Model3 requires the frozen five-loss weights",
    )
...
def create_side_model3_trainer(*, model, train_dataset, val_dataset=None, cfg):
    """Create the trainer that performs EMA only after executed optimizer steps."""

    from .trainer import SideModel3Trainer

    return SideModel3Trainer(
        model=model,
