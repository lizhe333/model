...
class SideModel3AdapterV2WAM(LightWAM):
    """Side-Model3-Adapter v2 with direct WM-to-action supervision."""

    method_id = "side_model3_adapter_v2_direct_wm_action_flow_v1"
    dyn_action_weight_max = 0.1
    dyn_action_warmup_steps = 1000
    ...
    def __init__(
        ...
        **lightwam_kwargs: Any,
        ...
        self.v2_local_optimizer_steps = 0
    ...
    def from_wan21_pretrained(
        ...
        *,
        ...
        if adapter_layers != cls.adapter_layer_indices:
            raise ValueError("Side-Model3-Adapter-v2 requires Wan adapters at layers 8,16,24")
        if int(adapter_config["adapter_dim"]) != cls.adapter_dim:
            raise ValueError("Side-Model3-Adapter-v2 requires adapter_dim=256")
        if float(adapter_config["adapter_scale"]) != cls.adapter_scale:
            raise ValueError("Side-Model3-Adapter-v2 requires adapter_scale=1.0")
        ...
        if not bool(getattr(video_expert, "use_wam_adapter", False)):
            raise ValueError("Side-Model3-Adapter-v2 did not instantiate Wan adapters")
        if tuple(getattr(video_expert, "adapter_layer_indices", ())) != cls.adapter_layer_indices:
            raise ValueError("Side-Model3-Adapter-v2 Wan adapter layers changed")
        if bool(getattr(video_expert, "has_backbone_lora", lambda: False)()):
            raise ValueError("Side-Model3-Adapter-v2 must not instantiate Wan LoRA")
        ...
        if tuple(side_config.get("layer_indices", cls.selected_wan_layers)) != cls.selected_wan_layers:
            raise ValueError("Side-Model3-Adapter-v2 requires Wan layers 8,16,20,24,29")
        side_config.update(
            {
    ...
    def update_ema_after_optimizer_step(self) -> None:
        ...
        self.v2_local_optimizer_steps += 1
        ...
        for layer_index, block in enumerate(self.video_expert.blocks):
            ...
            if layer_index in self.adapter_layer_indices:
                if use_target_adapters:
                    tokens = self.target_wan_adapters.apply_adapters(layer_index, tokens)
                else:
    ...
    def build_inputs(self, sample, tiled: bool = False) -> dict[str, Any]:
        ...
        packed_latents = sample.get("video_latents")
        if video is not None and packed_latents is not None:
            raise ValueError("Provide either raw video or Side-Model3-Adapter latents, not both")
        if packed_latents is not None:
            if packed_latents.ndim != 5 or int(packed_latents.shape[2]) != 3:
                raise ValueError(
                    "Side-Model3-Adapter cached latents must be [B,C,3,H,W] from independent encodes"
                )
            ...
            observation_latents = [
                packed_latents[:, :, position : position + 1]
                for position in range(3)
            ]
            batch_size = int(packed_latents.shape[0])
            ...
            raise ValueError("Side-Model3-Adapter requires sample['video_latents'] during training")
        ...
        future_valid = {
            4: torch.ones(batch_size, device=self.device, dtype=torch.bool),
            8: torch.ones(batch_size, device=self.device, dtype=torch.bool),
        }
    ...
    @staticmethod
    def _detached_functional_state(module: torch.nn.Module) -> dict[str, torch.Tensor]:
        """Snapshot module parameters/buffers without disabling input autograd."""
    ...
    def _predicted_action_state(
        ...
        *,
        ...
        horizon: int,
        action_encoder_state: dict[str, torch.Tensor],
        transition_state: dict[str, torch.Tensor],
    ) -> torch.Tensor:
        """Run branch-frozen E/T while retaining d(state)/d(predicted_action)."""

        action_tokens = functional_call(
            self.action_chunk_encoder,
            action_encoder_state,
            args=(predicted_clean_action,),
            kwargs={"horizon": horizon},
        )
        return functional_call(
            self.transition_predictor,
            transition_state,
            args=(control_state.detach(), action_tokens),
        )

    def _dyn_action_weight(self) -> float:
        return self.dyn_action_weight_max * min(
            1.0,
            self.v2_local_optimizer_steps / float(self.dyn_action_warmup_steps),
        )

    @staticmethod
    def _reconstruct_clean_action(
        ...
        *,
        ...
        sigma = (timestep / float(num_train_timesteps)).to(
            device=noisy_action.device,
            dtype=noisy_action.dtype,
        )
        return noisy_action - sigma.view(-1, 1, 1) * predicted_velocity

    @staticmethod
    def _weighted_masked_batch_mean(
        loss_per_sample: torch.Tensor,
        timestep_weight: torch.Tensor,
        valid: Optional[torch.Tensor],
    ) -> torch.Tensor:
        weights = timestep_weight.to(
            device=loss_per_sample.device,
            dtype=loss_per_sample.dtype,
        )
        if valid is None:
            return (loss_per_sample * weights).mean()
        mask = valid.to(device=loss_per_sample.device, dtype=loss_per_sample.dtype)
        return (loss_per_sample * weights * mask).sum() / mask.sum().clamp(min=1.0)
    ...
    def training_loss(self, sample, tiled: bool = False):
        ...
        predicted_clean_action = self._reconstruct_clean_action(
            noisy_action,
            predicted_action_velocity,
            timestep_action,
            num_train_timesteps=int(
                getattr(self.train_action_scheduler, "num_train_timesteps", 1000)
            ),
        )
        ...
        action_encoder_state = self._detached_functional_state(self.action_chunk_encoder)
        transition_state = self._detached_functional_state(self.transition_predictor)
        dyn_state_losses: dict[int, torch.Tensor] = {}
        for horizon in self.target_action_offsets:
            ...
            predicted_action_state = self._predicted_action_state(
            ...
            dyn_state_per_sample = self._future_state_loss_per_sample(
                predicted_action_state,
                target_state.detach(),
            )
            dyn_state_losses[horizon] = self._weighted_masked_batch_mean(
                dyn_state_per_sample,
                action_weight,
                inputs["future_valid"][horizon],
            )
        ...
        dyn_action_loss = (
            0.25 * dyn_state_losses[4]
            + 0.50 * dyn_state_losses[8]
        )
        total = sum(self.loss_weights[name] * value for name, value in losses.items())
        total = total + self._dyn_action_weight() * dyn_action_loss
        metrics = {
            f"loss_{name}": float(value.detach().item())
            for name, value in losses.items()
        }
        metrics["loss_dyn_action"] = float(dyn_action_loss.detach().item())
        metrics["dyn_action_weight"] = self._dyn_action_weight()
        metrics["v2_local_optimizer_steps"] = float(self.v2_local_optimizer_steps)
    ...
    def checkpoint_config(self) -> dict[str, Any]:
        return {
            ...
            "dyn_action_weight_max": self.dyn_action_weight_max,
            "dyn_action_warmup_steps": self.dyn_action_warmup_steps,
        }
    ...
    def save_checkpoint(self, path, optimizer=None, step=None):
        payload = {
            ...
            "v2_local_optimizer_steps": self.v2_local_optimizer_steps,
            "dyn_action_weight": self._dyn_action_weight(),
        }
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        self.v2_local_optimizer_steps = int(payload.get("v2_local_optimizer_steps", 0))
        expected_dyn_weight = self._dyn_action_weight()
        if float(payload.get("dyn_action_weight", expected_dyn_weight)) != expected_dyn_weight:
            raise ValueError("Side-Model3-Adapter-v2 warmup state mismatch")
        if optimizer is not None and "optimizer" in payload:
            optimizer.load_state_dict(payload["optimizer"])
        return payload

    def load_v1_model_only_warmstart(self, path) -> None:
        """Load a v1 Side-Model3-Adapter model without optimizer/scheduler state."""

        payload = torch.load(path, map_location="cpu")
        if payload.get("method_id") != "side_model3_adapter_wan_residual_ladder_flow_v1":
            raise ValueError("v2 warm start must come from Side-Model3-Adapter v1")
        if payload.get("model_class") != "SideModel3AdapterWAM":
            raise ValueError("v2 warm start must come from SideModel3AdapterWAM")
        self.mot.load_state_dict(payload["mot"], strict=True)
        self.video_expert.wam_adapters.load_state_dict(
            payload["online_wan_adapters"], strict=True
        )
