...
RUN_ID="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_side_model3_adapter_v2_object}"
...
export DATASET_DIR="${DATASET_DIR:-${PROJECT_ROOT}/Light-WAM/data/libero_mujoco3.3.2/libero_object_no_noops_lerobot}"
export TEXT_EMBED_CACHE_DIR="${TEXT_EMBED_CACHE_DIR:-${PROJECT_ROOT}/Light-WAM/data/text_embeds_cache/libero_wan21t2b}"
export LATENT_CACHE_DIR="${LATENT_CACHE_DIR:-${PROJECT_ROOT}/Light-WAM/data/latent_cache_Wan2.1-T2V-1.3B/libero_object_2cam224_side_model3_independent_v1}"
export WANDB_NAME="${WANDB_NAME:-side_model3_adapter_v2_object_${RUN_ID}}"
