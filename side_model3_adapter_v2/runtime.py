...
from .models import SideModel3AdapterV2WAM
...
def create_side_model3_adapter_v2_wam(
    *,
    model_id: str,
    ...
    side_model3_adapter_v1_warmstart_path: str | None = None,
    ...
    if state_fusion:
        raise ValueError("Side-Model3-Adapter-v2 cannot instantiate StateFusion")
    if not bool(adapter_config.get("use_wam_adapter", False)):
        raise ValueError("Side-Model3-Adapter-v2 requires Wan residual adapters")
    if bool(adapter_config.get("use_backbone_lora", False)):
        raise ValueError("Side-Model3-Adapter-v2 cannot instantiate Wan LoRA")
    _require_value(
        tuple(int(value) for value in adapter_config.get("adapter_layer_indices", ()))
        == (8, 16, 24),
        "Side-Model3-Adapter-v2 requires adapter layers 8,16,24",
    )
    _require_value(
    ...
    _require_value(
        ...
        "Side-Model3-Adapter-v2 requires first_frame_causal Wan attention",
    )

    _require_value(model_id == "Wan-AI/Wan2.1-T2V-1.3B", "Side-Model3-Adapter-v2 requires Wan2.1-T2V-1.3B")
    _require_value(video_backbone_type == "wan2_1_t2v", "Side-Model3-Adapter-v2 requires the wan2_1_t2v backend")
    _require_value(
    ...
    _require_value(
        ...
        == (8, 16, 20, 24, 29),
        "Side-Model3-Adapter-v2 requires Wan layers 8,16,20,24,29",
    )
    ...
    _require_value(
        ...
        "Side-Model3-Adapter-v2 keeps Action-DiT checkpointing disabled",
    )
    ...
    model = SideModel3AdapterV2WAM.from_wan21_pretrained(
    ...
    if side_model3_adapter_v1_warmstart_path:
        model.load_v1_model_only_warmstart(side_model3_adapter_v1_warmstart_path)
    return model


def create_side_model3_adapter_v2_trainer(*, model, train_dataset, val_dataset=None, cfg):
    """Create the trainer that performs EMA only after executed optimizer steps."""

    from .trainer import SideModel3AdapterV2Trainer

    return SideModel3AdapterV2Trainer(
        model=model,
