"""Hydra-facing factory for the top-level model3 package."""
...
def _as_dict(name: str, value: Any, *, required: bool = False) -> dict[str, Any]:
    ...
    if value is None:
        if required:
            raise ValueError(f"`{name}` is required for model3.")
        return {}
...
def create_model3_wam(
    *,
    ...
    """Build Model3WAM on the clean Light-WAM source vendored under model3."""
    ...
    if legacy_state_fusion:
        raise ValueError(
            "Model3 cannot consume a StateFusion action-expert config; "
            "use `action_query_policy_config`."
        )
    ...
    if missing_schedule:
        raise ValueError(
            "`action_scheduler` is missing required keys: "
            f"{sorted(missing_schedule)}"
        )
    ...
    return Model3WAM.from_wan22_pretrained(
        ...
        video_num_train_timesteps=int(
            video_schedule.get("num_train_timesteps", 1000)
        ),
        ...
        action_temporal_weighting_enabled=bool(
            temporal_weighting.get("enabled", False)
        ),
        action_temporal_weighting_num_prefix_steps=temporal_weighting.get(
            "num_prefix_steps"
        ),
