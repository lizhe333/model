...
HYDRA_CONFIG_ROOT="${HYDRA_CONFIG_ROOT:-${PROJECT_ROOT}/model3/configs/hydra}"
MODEL_PACKAGE_LABEL="${MODEL_PACKAGE_LABEL:-model3}"
cd "${BACKEND_ROOT}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/runs/I-003/model3/backend_runs/${RUN_ID}}"
...
USE_LATENT_CACHE="${USE_LATENT_CACHE:-true}"
...
if [[ "${USE_LATENT_CACHE}" != "true" && "${USE_LATENT_CACHE}" != "false" ]]; then
  echo "[launch] ERROR: USE_LATENT_CACHE must be true or false, got ${USE_LATENT_CACHE}" >&2
  exit 2
fi
if [[ "${USE_LATENT_CACHE}" == "true" && -z "${LATENT_CACHE_DIR}" ]]; then
  echo "[launch] ERROR: LATENT_CACHE_DIR is required when USE_LATENT_CACHE=true" >&2
  exit 2
fi
if [[ "${USE_LATENT_CACHE}" == "true" ]]; then
  DATASET_OVERRIDE_ARGS+=("data.train.latent_cache_dir=${LATENT_CACHE_DIR}")
else
  DATASET_OVERRIDE_ARGS+=("data.train.latent_cache_dir=null")
fi
...
echo "[launch] mode=${MODEL_PACKAGE_LABEL}_on_vendored_lightwam gpus=${GPU_IDS} num_processes=${NUM_PROCESSES} port=${MAIN_PROCESS_PORT}"
...
echo "[launch] use_latent_cache=${USE_LATENT_CACHE}"
...
CUDA_VISIBLE_DEVICES="${GPU_IDS}" accelerate launch \
  ...
  "data.train.use_latent_cache=${USE_LATENT_CACHE}" \
