...
from envs.utils.ffmpeg_utils import get_ffmpeg_executable
...
def _apply_eval_video_overrides(args, usr_args):
    if "eval_video_fps" in usr_args:
        args["eval_video_fps"] = int(usr_args["eval_video_fps"])
    if "eval_video_record_mode" in usr_args:
        args["eval_video_record_mode"] = str(usr_args["eval_video_record_mode"])
    if "eval_video_frame_stride" in usr_args:
        args["eval_video_frame_stride"] = int(usr_args["eval_video_frame_stride"])
...
def main(usr_args):
    ...
    with open(f"./task_config/{task_config}.yml", "r", encoding="utf-8") as f:
        args = yaml.load(f.read(), Loader=yaml.FullLoader)
    _apply_eval_video_overrides(args, usr_args)
