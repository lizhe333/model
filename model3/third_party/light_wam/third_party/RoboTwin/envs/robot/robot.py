...
class Robot:
    ...
    def reset(self, scene, need_topp=False, **kwargs):
        ...
        planner_missing = self.left_planner is None or self.right_planner is None
        conn_missing = self.left_conn is None or self.right_conn is None

        if self.communication_flag and not conn_missing:
            if self.left_conn:
            ...
            if planner_missing or not isinstance(self.left_planner, CuroboPlanner) or not isinstance(
