"""Model3 action policy: recurrent VLA-style queries with flow decoding."""
...
class LayerWiseRecurrentActionQueryEncoder(nn.Module):
    ...
    def forward(
        ...
        if self.proprio_projector is not None:
            if proprio is None:
                raise ValueError("proprio is required by the model3 action-query encoder")
...
class VLAQueryDiTActionExpert(nn.Module):
    ...
    method_id = "model3_vla_recurrent_query_flow_v1"
    ...
    def _validate_action(self, action: torch.Tensor, timestep: torch.Tensor) -> None:
        ...
        if int(action.shape[1]) != self.action_horizon:
            raise ValueError(
                f"model3 action horizon is fixed at {self.action_horizon}, got {action.shape[1]}"
            )
    ...
    def predict_velocity_from_queries(
        ...
        normalized_queries = self.query_condition_norm(query_memory.to(dtype=noisy_action.dtype)) #进行平均，全局调制，[B,64,512]
        query_summary = normalized_queries.mean(dim=1) #[B,512]
        t_mod = self.time_projection(time_embed) + self.query_modulation(query_summary)
        t_mod = t_mod.unflatten(1, (6, self.hidden_dim))

        tokens = self.action_encoder(noisy_action) #产生完整，注意力上下文 [B,8,512]
        context = self.query_context_projection(normalized_queries) #[B,64,512]
    ...
    def sample_from_queries(
        ...
        *,
        ...
        if int(action_horizon) != self.action_horizon:
            raise ValueError(
                f"model3 requires action_horizon={self.action_horizon}, got {action_horizon}"
            )
