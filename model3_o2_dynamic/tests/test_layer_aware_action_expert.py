...
from model3_o2_dynamic.models import (
...
def test_dynamic_gate_freeze_is_scoped_to_original_layer_readout() -> None:
    model = _minimal_wam(Model3O2DynamicWAM, _expert())
    ...
    model._response_adapters_pending_unfreeze = False
    model._o2_gate_trainable = True
    model._o2_gate_pending_unfreeze = False

    model.set_o2_gate_trainable(False)
    assert model.o2_gate_trainable is False
    assert all(not parameter.requires_grad for parameter in model.action_policy.layer_readout.parameters())
    assert model.action_policy.query_encoder.action_queries.requires_grad is True

    model.schedule_o2_gate_unfreeze()
    model._activate_pending_o2_gate_unfreeze()
    assert model.o2_gate_trainable is True
    assert all(parameter.requires_grad for parameter in model.action_policy.layer_readout.parameters())
