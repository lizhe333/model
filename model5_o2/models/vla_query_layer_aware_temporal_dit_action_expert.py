"""O2 layer-aware Action-DiT used with Model5 temporal feature grids."""
...
class VLAQueryLayerAwareTemporalDiTActionExpert(VLAQueryDiTActionExpert):
    """Preserve the O2 readout while accepting Model5 temporal layer states."""
    ...
    def __init__(
        ...
        *,
        ...
        if readout_type != "layer_separable_gated_residual":
            raise ValueError(f"unsupported Model5 O2 readout_type: {readout_type!r}")
        if readout_gate_type != "querywise_scalar":
            raise ValueError(
                f"unsupported Model5 O2 readout_gate_type: {readout_gate_type!r}"
            )
        if not readout_identity_init:
            raise ValueError("Model5 O2 requires exact q3 identity initialization")
        super().__init__(
